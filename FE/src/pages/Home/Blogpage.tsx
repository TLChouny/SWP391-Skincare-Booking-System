...
const CardTitle: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <h2 className={`text-xl font-semibold line-clamp-2 h-14 ${className}`}>{children}</h2>
);
...
const BlogPage: React.FC = () => {
  ...
  const featuredBlog = blogs.find((blog) => blog.createName === "LuLu Spa") || blogs[0] || null;
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2">
                  <div className="relative h-full">
                    <img
                      src={featuredBlog.image || "/placeholder.svg?height=300&width=400"}
                  ...
                  <div className="p-8 flex flex-col justify-between">
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900 mb-4">{featuredBlog.title}</h3>
                      <p className="text-gray-600 mb-6 line-clamp-4">{featuredBlog.description}</p>
                    </div>
                    <div className="mt-auto">
                      <div className="flex items-center mb-4">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900">{featuredBlog.createName}</p>
                          <p className="text-xs text-gray-500">
                            {new Date(featuredBlog.createdAt || Date.now()).toLocaleDateString("vi-VN", {
                              year: "numeric",
                              month: "long",
                              day: "numeric",
                            })}
            ...
            {blogs.length > 0 ? (
              blogs
                ...
                .map((blog) => (
                  ...
                  >
                    <img
                      src={blog.image || "/placeholder.svg?height=160&width=240"}
                      alt={blog.title}
                    ...
                    <CardFooter className="flex justify-between items-center mt-auto">
                      <span className="text-sm text-gray-500 truncate">{blog.createName}</span>
