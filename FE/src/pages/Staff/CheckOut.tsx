...
const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const calculateTotal = (): number => {
    return cart
      .filter((item) => item.status === "completed") // Filter for "completed" items
  ...
  const handleCheckout = async () => {
    ...
    if (completedItems.length === 0) {
      toast.error("Không có mục nào ở trạng thái 'completed' để thanh toán.");
      setShowModal(false);
      return;
    }

    const totalAmount = calculateTotal();
    const orderName = completedItems[0]?.serviceName || "Nhiều dịch vụ";
  ...
  const handlePayment = async () => {
    try {
      ...
      const completedItems = cart.filter((item) => item.status === "completed");
      if (completedItems.length === 0) {
        throw new Error("Không có mục nào ở trạng thái 'completed' để thanh toán.");
      }

      await Promise.all(
        completedItems.map(async (item) => {
          const response = await fetch(`${API_BASE_URL}/cart/${item.CartID}`, {
            method: "PUT",
            headers: {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Không thể cập nhật mục giỏ hàng ${item.CartID}: ${errorData.message || "Lỗi không xác định"}`
            );
          }
        })
      );
      ...
      toast.error(
        error instanceof Error ? error.message : "Lỗi khi cập nhật trạng thái thanh toán."
      );
    }
  };
