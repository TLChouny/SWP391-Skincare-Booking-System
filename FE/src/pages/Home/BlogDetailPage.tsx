...
const BlogDetailPage: React.FC = () => {
      ...
      try {
        ...
        if (data && typeof data === "object" && data._id) {
          setBlog({
            ...
            content: data.content || "No detailed content available.",
            description: data.description,
            image: data.image || "/placeholder.svg",
            createName: data.createName || "Unknown author",
        ...
        setError(error.message || "Could not load the blog post.");
        toast.error(error.message || "Error loading blog post.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!blog) {
    return (
      <Layout>
        <div className='min-h-screen flex items-center justify-center'>
          <p className='text-lg text-gray-500 italic'>Blog post not found.</p>
        </div>
      </Layout>
    );
  }
          ...
          <div className='prose prose-lg max-w-none bg-white p-8 rounded-xl shadow-md'>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: blog.content || "No detailed content available.",
              }}
            />
          </div>
